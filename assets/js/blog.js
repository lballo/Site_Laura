...
function setupEventListeners() {
  ...
  if (recoBtn) {
    recoBtn.addEventListener('click', handleRecommendation);
  }
}
...
function handleRecommendation() {
  const textarea = document.querySelector('#reco-input');
  const resultsDiv = document.querySelector('#reco-results');
  ...
  if (query === '') {
    alert('Décrivez votre problème pour obtenir des recommandations.');
    return;
  }
  
  const recommendations = searchArticles(query).slice(0, 3);
  
  if (recommendations.length === 0) {
    resultsDiv.innerHTML = '<p style="color: var(--gray-brown);">Aucune recommandation trouvée. Essayez d\'autres mots.</p>';
    return;
  }
  
  let html = '<h4>Articles recommandés pour vous :</h4>';
  
  recommendations.forEach(article => {
    html += `
      <a href="${article.url}" class="reco-card">
        <div class="reco-card-title">${article.title}</div>
        <div class="reco-card-match">Pertinent pour vous</div>
      </a>
    `;
  });
